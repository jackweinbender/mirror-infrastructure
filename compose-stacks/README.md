...
`docker-host/` is the platform stack: it owns Traefik and the external `proxy` network and is reconciled first by `deploy.yaml` on every deploy host. It is never removed by application reconciliation.
...
The external `proxy` network is provided by the platform phase of the deployment workflow. Application Compose files that declare it as external run only after that phase succeeds; application reconciliation never removes it.
