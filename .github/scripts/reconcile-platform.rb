...
cleanup = proc { ScriptCommands.ssh(remote, "rm -rf -- #{ScriptCommands.quote(stage)}") }
at_exit { cleanup.call }
error = proc { |message| ScriptOutput.fail!(message, prefix: "[#{host_id}/docker-networking]") }

error.call('unable to create remote staging directory') unless ScriptCommands.ssh(remote, "umask 077 && mkdir -p -- #{ScriptCommands.quote(base + '/.staging')} && mkdir -- #{ScriptCommands.quote(stage)}")
error.call('unable to prepare external proxy network') unless ScriptCommands.ssh(remote, 'docker network inspect proxy >/dev/null 2>&1 || docker network create --driver bridge --attachable proxy >/dev/null')
...
error.call('unable to prepare platform environment') unless ScriptCommands.ssh(remote, "umask 077 && : > #{ScriptCommands.quote(stage + '/.env')} && chmod 600 #{ScriptCommands.quote(stage + '/.env')}")
error.call('staged Compose validation failed') unless ScriptCommands.ssh(remote, "cd -- #{ScriptCommands.quote(stage)} && docker compose --project-name docker-networking --env-file .env config --quiet")
...
error.call('existing platform directory is unmarked or has the wrong marker; refusing replacement') unless ScriptCommands.ssh(remote, marker_check)
error.call('platform publication failed') unless ScriptCommands.ssh(remote, "if [ -e #{ScriptCommands.quote(live)} ]; then rm -rf -- #{ScriptCommands.quote(live)}; fi && mv -- #{ScriptCommands.quote(stage)} #{ScriptCommands.quote(live)}")
error.call('proxy network is unavailable after platform publication') unless ScriptCommands.ssh(remote, 'docker network inspect proxy >/dev/null 2>&1')
...
error.call('legacy docker-host migration failed') unless ScriptCommands.ssh(remote, legacy_cleanup)
error.call('unable to ensure external proxy network after legacy migration') unless ScriptCommands.ssh(remote, 'docker network inspect proxy >/dev/null 2>&1 || docker network create --driver bridge --attachable proxy >/dev/null')
